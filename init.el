...
(defun my/setup-frame (&optional frame)
  "Configure look and geometry of FRAME."
...
(use-package consult
  ...
  ;; `consult-register-store' and the built-in commands.
  ...
  ;; Use rga if available, otherwise regular rg
  (when (executable-find "rga")
    (setq consult-ripgrep-args (replace-regexp-in-string "rg " "rga " consult-ripgrep-args))))
    ...
    (let* ((absolute-pdf (expand-file-name pdf-file))
      ...
      (if (string-blank-p (string-trim pdf-text))
        ...
        (let ((prompt (concat "Extract bibliographic metadata from the provided PDF text.\n"
                              ...
                              "CRITICAL RULES:\n"
                              "1. Determine the correct BibLaTeX entry type based on the text (e.g., @article, @book, @incollection, @inproceedings, @phdthesis, @misc).\n"
                              "2. Use only ONE hyphen for page ranges (e.g., pages={255-285}, not 255--285).\n"
                              "3. Use sentence case for titles, but keep journal/book names in Title Case.\n"
                              "4. Protect given names and proper nouns in titles with curly braces.\n"
                              "5. Include ALL available metadata in the bibtex string (journal, booktitle, volume, number, pages, doi, publisher, editor, etc.).\n"
                              "6. Do NOT include 'abstract' or 'keywords'.\n\n"
                              ...
                              "  \"bibtex\": \"@<ENTRY_TYPE>{Lastname_YYYY, author = {First Last}, year = {YYYY}, title = {Title}, ...[add all other relevant fields here]... }\"\n"
                              "}\n\n"
                              "TEXT TO ANALYZE:\n"
                              pdf-text)))
